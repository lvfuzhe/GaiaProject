...
默认的 `standard-v9` 规则引擎已覆盖六轮主流程、完整建筑链、六条科研轨、能量碗、
...
src/gaiazero/game/gaia_state.py standard-v9 AI 规则核心
...
完整 QIC 自由转换、失落星球和部分种族特殊行动仍未实现。训练结果应视为 `standard-v9` 环境内的策略，不应直接
作为官方比赛裁决器。`standard-v9` 统一了 BGA 板块编号、科研逐级收益、初始科研收益、公共能量与 Q.I.C. 行动、学院类型及助推特殊行动；旧版检查点的状态语义不同，不能直接沿用。
