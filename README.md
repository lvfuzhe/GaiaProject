...
默认的 `standard-v7` 规则引擎已覆盖六轮主流程、完整建筑链、六条科研轨、能量碗、
...
src/gaiazero/game/gaia_state.py standard-v7 AI 规则核心
...
| 玩家数 + 3 个助推板块、末位到首位开局选取、过轮归还/重选 | 已实现 |
...
完整 QIC/自由行动和失落星球仍未实现。训练结果应视为 `standard-v7` 环境内的策略，不应直接
作为官方比赛裁决器。`standard-v7` 统一了 BGA 板块编号、科研逐级收益及初始科研收益；旧版检查点的状态语义不同，不能直接沿用。
