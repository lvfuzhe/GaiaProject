...
默认的 `standard-v4` 规则引擎已覆盖六轮主流程、完整建筑链、六条科研轨、能量碗、
...
src/gaiazero/game/gaia_state.py standard-v4 AI 规则核心
...
完整 QIC/自由行动和失落星球仍未实现。训练结果应视为 `standard-v4` 环境内的策略，不应直接
作为官方比赛裁决器。`standard-v4` 的观察与动作维度已经变化，旧版检查点不能直接加载。

看板的初始设置路径为 `http://127.0.0.1:8765/setup/random`；需要逐块调整星区时使用
`http://127.0.0.1:8765/setup/manual`，也可以在页面内切换 BGA 随机与手动星区路径。手动路径
支持选中任意单颗星球并将其放置到整张拼接星图的合法空六角格，提交预览时会检查重叠、边界和
同色母星相邻限制。
